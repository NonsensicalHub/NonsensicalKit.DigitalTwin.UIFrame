...
namespace ZTools
{
    ...
    public class ZCalendar : MonoBehaviour
    {
        ...
        public void RangeCalendar(DateTime firstDay, DateTime secondDay)
        {
            ...
            if (isFirstSelect)
            {
                firstTime.Item1 = zCalendarModel.hour.choiceTime;
                firstTime.Item2 = zCalendarModel.min.choiceTime;
                firstTime.Item3 = zCalendarModel.second.choiceTime;
            }

            m_RangeTimeEvent?.Invoke(firstDay.AddHours(firstTime.Item1).AddMinutes(firstTime.Item2).AddSeconds(firstTime.Item3), secondDay.AddHours(zCalendarModel.hour.choiceTime).AddMinutes(zCalendarModel.min.choiceTime).AddSeconds(zCalendarModel.second.choiceTime));
