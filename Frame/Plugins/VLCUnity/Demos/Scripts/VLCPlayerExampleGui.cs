...
{
	[FormerlySerializedAs("vlcPlayer")] public CustomVLCPlayer customVlcPlayer;
	...
	{
		//VLC Event Handlers
		customVlcPlayer.mediaPlayer.Playing += (object sender, EventArgs e) => {
		...
		customVlcPlayer.mediaPlayer.Paused += (object sender, EventArgs e) => {
			//Always use Try/Catch for VLC Events
			try
			{
		...
		customVlcPlayer.mediaPlayer.Stopped += (object sender, EventArgs e) => {
			//Always use Try/Catch for VLC Events
			try
			{
				_isPlaying = false;//Switch to the Play button next update
		...
		//Buttons
		pauseButton.onClick.AddListener(() => { customVlcPlayer.Pause(); });
		playButton.onClick.AddListener(() => { customVlcPlayer.Play(); });
		stopButton.onClick.AddListener(() => { customVlcPlayer.Stop(); });
		...
		openButton.onClick.AddListener(() => { customVlcPlayer.Open(pathInputField.text); });

		UpdatePlayPauseButton(customVlcPlayer.playOnAwake);
		...
		seekBarPointerUp.callback.AddListener((data) => { 
			...
			customVlcPlayer.SetTime((long)((double)customVlcPlayer.Duration * seekBar.value));
		});
		seekBarEvents.triggers.Add(seekBarPointerUp);

		//Path Input
		pathInputField.text = customVlcPlayer.path;
		...
		volumeBar.value = customVlcPlayer.Volume;
		volumeBar.onValueChanged.AddListener((data) => { customVlcPlayer.SetVolume((int)volumeBar.value);	});
	...
	void Update()
	{
		...
		if(customVlcPlayer.texture != null)
			screenAspectRatioFitter.aspectRatio = (float)customVlcPlayer.texture.width / (float)customVlcPlayer.texture.height;
	...
	void UpdateSeekBar()
	{
		...
		{
			var duration = customVlcPlayer.Duration;
			if (duration > 0)
				seekBar.value = (float)((double)customVlcPlayer.Time / duration);
		}
	}
	...
	{
		buttonList.Clear();
		var tracks = customVlcPlayer.Tracks(type);
		var selected = customVlcPlayer.SelectedTrack(type);
